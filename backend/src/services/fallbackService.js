...
module.exports = { triggerFallback };const EmergencySession = require('../models/EmergencySession');
const { getSingleETA } = require('./mapsService');
const { getAvailableAmbulancesNear } = require('./ambulanceCache');
const redis = require('../config/redis');
const logger = require('../utils/logger');
...
async function fallbackLevel3and4(session, currentEta) {
  ...
  try {
    ...
    // In production: await fetch(hospital.webhookUrl, { method: 'POST', body: JSON.stringify(payload) })
    // Simulated here — logs the payload and writes to eventLog
    logger.warn(`Fallback L4: hospital webhook triggered`, payload);
